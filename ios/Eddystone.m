...
- (instancetype)init {
    if ((self = [super init]) != nil) {
        ...
        _eddystoneModule =[Eddystone init];
    }
 ...
 didDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    if(hasListeners){
        ...
        if (frameType == FrameTypeUID || frameType == FrameTypeEID) {
            ...
            [_eddystoneModule emitOnURLFrame:@{
                @"uid": [peripheral.identifier UUIDString],
            ...
            if (beaconData) {
                if ([beaconData length] > 1) {
                    ...
                    temp += (bytes[5] & 0xFF);
                    temp /= 256.f;
                    [_eddystoneModule emitOnTelemetryData:@{
                        @"uid": [peripheral.identifier UUIDString],
            ...
            [_eddystoneModule emitOnEmptyFrame:@{}];
...
- (void)centralManagerDidUpdateState:(nonnull CBCentralManager *)manager {
    switch(manager.state) {
        case CBManagerStatePoweredOn:
            ...
            [_eddystoneModule emitOnStateChanged:@{@"value":@"on"}];
        ...
        case CBManagerStatePoweredOff:
            [_eddystoneModule emitOnStateChanged:@{@"value":@"off"}];
            break;
            
        case CBManagerStateResetting:
            [_eddystoneModule emitOnStateChanged:@{@"value":@"resseting"}];
            break;
            
        case CBManagerStateUnsupported:
            //[self sendEventWithName:@"onStateChanged" body:@"unsupported"];
            [_eddystoneModule emitOnStateChanged:@{@"value":@"unsupported"}];
            
            break;
            
        case CBManagerStateUnauthorized:
            //[self sendEventWithName:@"onStateChanged" body:@"unauthorized"];
            [_eddystoneModule emitOnStateChanged:@{@"value":@"unauthorized"}];
            
            break;
            
        default:
            // [self sendEventWithName:@"onStateChanged" body:@"unknown"];
            [_eddystoneModule emitOnStateChanged:@{@"value":@"unknown"}];
